<template>
    <el-dialog class="monthly-calculation-dialog" fullscreen :visible="open" @close="close" style="margin-top: -2rem; padding-top: 0rem; top: 0;background: rgba(255, 255, 255, .99);" width="85vw">
        <el-row>
            <h1>Calculer vos mensualités</h1>
        </el-row>
        <el-row>
                 ...
                 <el-tab-pane v-for="(simulation, index) in simulations" :closable="simulations.length > 1" :name="simulation.id">
                     ...
                     <el-row style="width: 100%">
                         <el-col  :xs="{span: 24, offset: 0}" :sm="{span: 24, offset: 0}" :md="{span: 12, offset: 0}" :lg="{span: 12, offset: 0}" :xl="{span: 12, offset: 0}" class="form-container">
                             <el-row class="case-title">Montant du prêt</el-row>
                             <el-row class="case-body">
                                   <el-input class="case-input-number" onClick="this.select()" placeholder="100 000" autocomplete="off" type="text" v-model="simulation.loanAmount" @change="formatAmount">
                             ...
                             <el-row class="case-body">
                                   <el-input class="case-input-number" autocomplete="off" onClick="this.select()" :placeholder="1" type="number" v-model="simulation.interestRate">
                                        <template slot="append">%</template>
                                    </el-input>
                                    <vue-slider
                                        ref="slider2"
                         ...
                         <el-col :xs="{span: 20, offset: 0}" :sm="{span: 20, offset: 0}" :md="{span: 12, offset: 0}" :lg="{span: 12, offset: 0}" :xl="{span: 12, offset: 0}" class="chart-container">
                             <el-row :class="'chart-blur-'+simulationProgress.toString()">
                                 <el-col :span="8" :offset="4">
                                     <el-row class="case-title">Vos mensualités</el-row>
                                     <el-row class="loan-price">{{((Number(monthlyChartData.datasets[0].data[0]) + Number(monthlyChartData.datasets[0].data[1])).formatMoney(0) === "1")?'0': (Number(monthlyChartData.datasets[0].data[0]) + Number(monthlyChartData.datasets[0].data[1])).formatMoney(0) }} €/mois</el-row>
                                     <el-row class="insurance-price">dont {{ (Number(monthlyChartData.datasets[0].data[1]).formatMoney(0) === '1')?'0':Number(monthlyChartData.datasets[0].data[1]).formatMoney(0) }} € d’assurance</el-row>
                                     <el-row>
                                         <doughnut-chart :data="monthlyChartData" :options="chartOptions" />
                                     </el-row>
                                ...
                                <el-col :span="8" :offset="4">
                                     <el-row class="case-title">Coût total du prêt</el-row>
                                     <el-row class="loan-price">{{ ((Number(totalChartData.datasets[0].data[0]) + Number(totalChartData.datasets[0].data[1])).formatMoney(0) === "1")?'0': (Number(totalChartData.datasets[0].data[0]) + Number(totalChartData.datasets[0].data[1])).formatMoney(0)}} €</el-row>
                                     <el-row class="insurance-price">dont {{ (Number(totalChartData.datasets[0].data[1]).formatMoney(0) === '1')?'0':Number(totalChartData.datasets[0].data[1]).formatMoney(0)}} € d’assurance</el-row>
                                     <el-row>
...
export default {
    ...
    computed: {
        ...
        totalChartData: function() {
            ...
            if (Number(simulation.loanAmount.split(' ').join('')) === 0 || simulation.loanDuration === '0' | simulation.interestRate === '0') {
                return {
                    labels: ['Crédit', 'Assurance'],
                    datasets: [
                        {
                ...
                monthlyPayment = Number(simulation.loanAmount.split(' ').join('')) * ((simulation.interestRate / 100) / 12);
                monthlyPayment = monthlyPayment / (1 - Math.pow(1 + ((simulation.interestRate / 100) / 12), -(simulation.loanDuration * 12)));
                monthlyPayment = monthlyPayment * (simulation.loanDuration * 12);
                insurancePayment = (Number(simulation.loanAmount.split(' ').join('')) * (simulation.insuranceRate / 100)) * simulation.loanDuration;
                return {
                    labels: ['Crédit', 'Assurance'],
                    datasets: [
                        {
                            backgroundColor: ['#212D59', 'rgba(71,81,117,0.45)'],
                            data: [monthlyPayment.toFixed(0), insurancePayment.toFixed(0)],
                            borderWidth: [2, 2]
                        }
                    ]
                };
        ...
        monthlyChartData: function() {
            ...
            if (Number(simulation.loanAmount.split(' ').join('')) === 0 || simulation.loanDuration === '0' | simulation.interestRate === '0') {
                ...
                monthlyPayment = Number(simulation.loanAmount.split(' ').join('')) * ((simulation.interestRate / 100) / 12);
                monthlyPayment = monthlyPayment / (1 - Math.pow(1 + ((simulation.interestRate / 100) / 12), -(simulation.loanDuration * 12)));
                insurancePayment = (Number(simulation.loanAmount.split(' ').join('')) * (simulation.insuranceRate / 100)) / 12;
                return {
                    labels: ['Crédit', 'Assurance'],
                    datasets: [
                        {
        ...
        validate() {
            if (this.simulationProgress > 3) {
                ...
                const requestBody = { 
                    ...
                    loanAmount: Number(simulationToSave.loanAmount.split(' ').join('')),
                ...
                this.$http.post(this.$root.URL + '/credit/save-payment-simulation', requestBody).then(res => {
                ...
                this.$message.error('Simulation incomplète');
            }
        },
        formatAmount(value) {
            const index = this.simulations.findIndex((item) => item.id === this.editableTabsValue);
            this.simulations[index].loanAmount = Number(value.split(' ').join('')).formatMoney(0);
        },
        addTab(targetName) {
            let newTabName = 1;
        ...
        removeTab(targetName) {
            if(this.simulations.length > 1) {
                ...
                if (!removedTab.created) {
                    this.$http.post(this.$root.URL + '/credit/remove-payment-simulation', { id: removedTab._id }).then(res => {
                        if (res.data.success) {
