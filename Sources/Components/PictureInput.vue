<template>
    <div ref="container" id="picture-input" class="picture-input">
        ...
        <div v-else-if="supportsPreview">
            <div class="preview-container" :style="{maxWidth: previewWidth + 'px', height: previewHeight + 'px', borderRadius: radius + '%'}">
                <canvas ref="previewCanvas"class="picture-preview" :class="computedClasses" @drag.stop.prevent="" @dragover.stop.prevent="" @dragstart.stop.prevent="onDragStart" @dragenter.stop.prevent="onDragStart" @dragend.stop.prevent="onDragStop" @dragleave.stop.prevent="onDragStop" @drop.stop.prevent="onFileDrop" @click.prevent="onClick" :style="{height: previewHeight + 'px', zIndex: zIndex + 1 }">
                </canvas>
                <div v-if="!imageSelected && !plain" class="picture-inner" :style="{top: -previewHeight + 'px', marginBottom: -previewHeight + 'px', fontSize: fontSize, zIndex: zIndex + 2}">
            ...
            this.getEXIFOrientation(file, orientation => {
                ...
                reader.onload = e => {
                    ...
                    this.imageObject.onload = () => {
                        ...
                        this.imageRatio = this.imageObject.width / this.imageObject.height
                        if (prefill) {
                            this.$emit('prefill')
                        } else {
                            this.$emit('change', this.image)
